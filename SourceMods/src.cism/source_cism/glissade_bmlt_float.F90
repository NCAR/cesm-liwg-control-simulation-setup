       ...
       do j = 1, nsn
          do i = 1, ewn
             ...
             if (floating_mask(i,j) == 1 .or. ocean_mask(i,j) == 1) then   ! ice is present and floating, or ice-free ocean
               ...
               !!! Modification added by SLB and MP to the linear shelf melt rates for grid points lower than .ca 70N
               !! This was to restrict the growth of large shelves across the southern region of the ice sheet
               !! The melt rate increases linearly from 10 m/yr at the depth of -50m to 100 m.yr at bmlt_float_depth_meltmax

	       if (jglobal < 360) then   

                if (z_draft > -50.) then     !Warm values: from 10 to 100 m/yr
                   bmlt_float(i,j) = 10.0/(315.36e05)    ! no freezing and minimum melting of 10 m/yr
                elseif (z_draft > basal_melt%bmlt_float_depth_zmeltmax) then
                   ! melting with a linear taper from meltmax to -3 m/yr
                   bmlt_float(i,j) = 10.0/(315.36e05) + ((-50. - z_draft)/ &
                   (-50. - basal_melt%bmlt_float_depth_zmeltmax))* &
                   (basal_melt%bmlt_float_depth_meltmax - 10.0/(315.36e05))
